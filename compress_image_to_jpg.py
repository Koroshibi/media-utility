...
def main():
    ...
    # Si plusieurs fichiers sont fournis, créer un dossier "compressed"
    create_compressed_folder = len(input_files) > 1
    compressed_folder = ""
    
    if create_compressed_folder:
        # Utiliser le dossier du premier fichier comme emplacement du dossier "compressed"
        base_dir = os.path.dirname(input_files[0])
        compressed_folder = os.path.join(base_dir, "compressed")
        if not os.path.exists(compressed_folder):
            os.makedirs(compressed_folder)
        print(f"Created output folder: {compressed_folder}")
    ...
    for input_path in input_files:
        ...
        if create_compressed_folder:
            output_path = os.path.join(compressed_folder, f"{name_without_ext}.jpg")
        else:
            base_dir = os.path.dirname(input_path)
            output_path = os.path.join(base_dir, f"{name_without_ext}_compressed.jpg")
